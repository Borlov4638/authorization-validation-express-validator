...
export const postBlogIsExistsById = async (req: Request, res: Response, next:NextFunction) => {
...
export const postIsExistsById = async (req: Request, res: Response, next:NextFunction) => {
    ...
    const postToFetch = await client.db("incubator").collection("posts").findOne({id: req.params.id})
    if(!postToFetch){
        return res.sendStatus(404)
    }
    return next()
