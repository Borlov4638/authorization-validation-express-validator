...
export const authNewPasswordValidation = () => body('newPassword').exists().withMessage({message:"password is not passed", field:"password"}).isString().isLength({min:6, max:20}).withMessage({message:"password is invalid", field:"password"})
